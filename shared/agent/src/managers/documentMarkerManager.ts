...
export class DocumentMarkerManager {
    ...
    private _codemarkDocumentMarkersCache = new Map<string,
        {
    ...
    private _debouncedDocumentMarkersChangedByReason = new Map<"document" | "codemarks",
            ...
            for (const marker of markersForDocument) {
                try {
                    ...
                    if (!locations[marker.id]) {
                        const contents = gitSha ? await git.getFileContentForRevision(documentId.uri, gitSha) : doc && doc.getText();
                        if (contents) {
